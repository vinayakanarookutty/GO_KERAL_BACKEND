...
import { Body, Controller, Post, Get,UseGuards,Req, HttpException, HttpStatus ,InternalServerErrorException} from "@nestjs/common";
...
console.log("Haiiiiiii")
    ...
    async driverLogin(@Body() body : {email : string , password : string}) {
        try {
            ...
            const checkPassword = await bcrypt.compare(body.password , ((await userData).password))
            if(!checkPassword){ 
    ...
    async updateDriver(@Body() body: { imageUrl: string; },@Req() req: Request) {
      try {
        ...
        const updatedDriver = await this.driverService.updateDriverImage(userId, imageUrl);
        if (!updatedDriver) {
          throw new HttpException("Driver not found", HttpStatus.NOT_FOUND);
        }
  
        return { message: "Profile picture updated successfully", driver: updatedDriver };
      } catch (error) {
    ...
    @UseGuards(AuthMiddleware)
    async updateDriverPersonalInfo(@Body() body: { personalInfo: any }, @Req() req: Request) {
      try {
